	...
	parameter(Nsource=1500,Nin=1000,Nout=1000,Ndet=1500,endrun=1,blknum=2)	
	...
	Nx=idnint(dephase/(delx))!this is how many gaussians are on the grating
...
!------------random widthblock on grating ------------------------	 
		...
		call randomblock(idum,blknum,blknumm,Nin,blkwid)
...
!wavephiout will change in every loop			
				...
				do r=1,Nx
					...
					do p=1,blknumm	
						...
						call decohere(decfactor,wavephiout,wavephiin,Nin,Nout)
...
do i=1,Nout
	wavephiout(i)=wavephiin(i)*decfactor(i)
	!write(6,*) wavephiout(i)
enddo
