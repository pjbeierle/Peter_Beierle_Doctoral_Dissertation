...
!====================================================================
		...
		OPEN(UNIT=14,FILE='electrondistribution_levinson'//trim(x1)//'.dat')
		OPEN(UNIT=15,FILE='off_diagonal_dist_levinson'//trim(x1)//'.dat')
...
!        do s2=1,1281
	 ...
	 do s2=start_local,end_local
	  call propagate(trajectory,finalheight,s1,s2,counter,sucesses,flag,gratingarray)
	   if (flag.eq.0) then
	    call levinsondecoherence(trajectory,reduced_diagonal,rho_1p5)
...
!-------------------------------------------------------------------------------
subroutine levinsondecoherence(trajectory,reduced_diagonal,rho_1p5)
implicit none
...
lambdadb=2*Pi*hbar/dsqrt(2.0d0*me*kbolt*Tkelvin)
