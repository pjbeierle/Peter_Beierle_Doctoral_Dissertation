...
!====================================================================
	...
	!	OPEN(UNIT=13,FILE='debug.dat')
		OPEN(UNIT=14,FILE='electrondistribution_mach'//trim(x1)//'.dat')
	!	OPEN(UNIT=15,FILE='off_diagonal_dist_mach'//trim(x1)//'.dat')
...
!	deflection=
!	surfheight=1.0d0	!-.59d-6
        surfheight=-.59d-6
      

	b=3.0d0
	!deltav=5843.0d0*exp(-((s2-1.0d0)**2)/(2.0d0*b**2))-5843.0d0*exp(-((s2-1281.0d0)**2)/(2.0d0*b**2))
	deltav=20.0d0*(-584.3d0*exp(-((s2-1.0d0))/(2.0d0*b**2))+584.3d0*exp(((s2-1281.0d0))/(2.0d0*b**2)))


      y(1)=0d0
      y(2)=v0*dcos(theta)		!initial x velocity of beam	
      y(3)=slit1position+(31d-2)*((slit2position-slit1position)/(25d-2))+deltav*((6d-2)/v0) 
	!initial y coordinate of beam,curved (1.67keV)
      y(4)=v0*dsin(theta)+deltav 		!initial y velocity of beam
	  ...
	  tend=tft						  
	   ...
	   !y(2)=y(2)				
	    ...
	    y(4)=y(4)+(yprime(4)*delt)
          yprime(4)=-k*q*q/(4*(y(3)+surfheight)*(y(3)+surfheight)*me)
	    surfheight=-.59d-6 !+(4*3.33333d-6)*(tft/endoftim)
