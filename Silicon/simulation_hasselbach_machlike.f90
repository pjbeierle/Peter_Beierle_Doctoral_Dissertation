...
!====================================================================
		...
		OPEN(UNIT=13,FILE='debug.dat')
                OPEN(UNIT=14,FILE='electrondistribution_machlike_lb'//trim(x1)//'.dat')
                OPEN(UNIT=15,FILE='off_diagonal_dist_machlike_lb'//trim(x1)//'.dat')
...
f=(4.13d0-2.01d0)/500d0 		!500 time steps (change as necessary)
Pi=3.14159265d0
...
do m=1,1
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
	gamma=(Pi/16.0d0)*((i*(5d-6)/1000d0)-(j*(5d-6)/1000d0))**2
	hassexp=dexp(-f*gamma/(trajectory(m)**2))
