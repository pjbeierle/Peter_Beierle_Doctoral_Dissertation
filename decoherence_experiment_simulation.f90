...
!    OPEN(UNIT=15,FILE='off_diagonal_dist_scheel456'//trim(x1)//'.dat')
...
!    start_local = N_local*myid + 1 + 732
!    end_local =  N_local*myid + N_local + 732
...
!====================================================================
		...
		OPEN(UNIT=13,FILE='debug.dat')
                OPEN(UNIT=15,FILE='off_diagonal_dist_howie456'//trim(x1)//'.dat')
	 ...
	 do s1=1,1901
	  ...
	  call propagate(trajectory,finalheight,s1,s2,counter,sucesses,flag,gratingarray)
	   if (flag.eq.0) then
	    ...
	    call howiedecoherence(trajectory,reduced_diagonal,rho_1p5)		
		!call scheeldecoherence(trajectory,reduced_diagonal,rho_1p5)	
		!call zurekdecoherence(trajectory,reduced_diagonal,rho_1p5)
		!call machdecoherence(trajectory,reduced_diagonal,rho_1p5)
...
!-------------------------------------------------------------------------------
subroutine howiedecoherence(trajectory,reduced_diagonal,rho_1p5)
implicit none
...
real*8 delt
integer i,j,m
real*8 Pi,hbar,h,kbolt,delx
real*8 resistivity,resistivitygold,resistivitysilicon
real*8 conductivity
real*8 inversezurektime,P,numerator,coefficient,expintegral
real*8 A,B,howie,nu
real*8 omegam

omegam=.6d12 !for semiconductors

delt=length/v0/500d0 	!assuming 500 steps (see propogation)
resistivitygold=2.2d-8
resistivitysilicon=.01d0
resistivity=resistivitysilicon !parameter
...
coefficient=(q*q*length*omegam*omegam)/(4.0d0*Pi*Pi*hbar*conductivity*v0*v0)
...
do m=1,1
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
        delx=abs((i*(5d-6)/1000d0)-(j*(5d-6)/1000d0))
        if(delx == 0.0d0) then
          P=0.0d0
        endif
        if(delx /= 0.0d0) then
          nu=trajectory(m)/(4.0d0*delx)
          A=dlog((.65d0+(0.56146d0/nu))*(1+nu))
          B=(nu**4)*exp(7.7d0*nu)*(2.0d0+nu)**(3.7)
          expintegral=((A**(-7.7))+B)**(-.13)
          P=coefficient*expintegral
        endif
	howie=dexp(-P/500d0)
	reduced_diagonal(i)=rho_1p5(i,j)*howie
 enddo
enddo
 ...
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
	...
	if(delx /= 0.0d0) then
        ...
        howie=dexp(-P/500d0)
        reduced_diagonal(i)=reduced_diagonal(i)*howie
 enddo
enddo
...
!----------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
subroutine scheeldecoherence(trajectory,reduced_diagonal,rho_1p5)
implicit none
...
real*8 delt
integer i,j,m
real*8 Pi,hbar,h,kbolt,Tkelvin,epsnot
real*8 resistivity,resistivitygold,resistivitysilicon
real*8 coefficient,gamma,scheel,bracket,xsquared
real*8 rprimenot
...
rprimenot=(2.0d0*epsnot)*resistivity

coefficient=(q*q*kbolt*Tkelvin*rprimenot)/(2.0d0*Pi*epsnot*hbar*hbar)
...
do m=1,1
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
	xsquared=((i*(5d-6)/1000d0)-(j*(5d-6)/1000d0))**2
    bracket=(1/(2.0d0*trajectory(m)))-1/sqrt((4.0d0*trajectory(m)**2)+xsquared)	
	gamma=-delt*coefficient*bracket
	scheel=dexp(gamma)
	reduced_diagonal(i)=rho_1p5(i,j)*scheel
 enddo
enddo
...
do m=2,500
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
	...
	gamma=-delt*coefficient*bracket
	scheel=dexp(gamma)
	reduced_diagonal(i)=reduced_diagonal(i)*scheel
 enddo
enddo
...
!----------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
subroutine zurekdecoherence(trajectory,reduced_diagonal,rho_1p5)
implicit none
...
real*8 Pi,hbar,h,kbolt,Tkelvin
real*8 resistivity,resistivitygold,resistivitysilicon
real*8 inversezurektime,zurek,numerator
...
do m=1,1
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
	numerator=(Pi*(q**2)*kbolt*Tkelvin*resistivity*((i*(5d-6)/1000d0)-(j*(5d-6)/1000d0))**2)
        inversezurektime=numerator/((4d0*h**2)*(trajectory(m)**3))	
	zurek=dexp(-delt*inversezurektime)
	reduced_diagonal(i)=rho_1p5(i,j)*zurek
 enddo
enddo
...
do m=2,500
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
        ...
        inversezurektime=numerator/((4d0*h**2)*(trajectory(m)**3))
        zurek=dexp(-delt*inversezurektime)
	reduced_diagonal(i)=reduced_diagonal(i)*zurek
 enddo
enddo
...
!----------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
subroutine machdecoherence(trajectory,reduced_diagonal,rho_1p5)
implicit none
...
real*8 Pi,hbar,h,kbolt,Tkelvin,epsnot
real*8 resistivity,resistivitygold,resistivitysilicon
real*8 coefficient,mach,xsquared
real*8 lambdadb,taur_inverse,taud_inverse
...
lambdadb=2.0d0*Pi*hbar/dsqrt(2.0d0*me*kbolt*Tkelvin)

coefficient=(q**2)/(2*Pi*epsnot*epsi*hbar*kfermi)
...
do m=1,1
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
	xsquared=((i*(5d-6)/1000d0)-(j*(5d-6)/1000d0))**2
	taur_inverse=coefficient/(trajectory(m)**2)
	taud_inverse=(Pi/32.0d0)*taur_inverse*xsquared/(lambdadb)**2
	mach=dexp(-delt*taud_inverse)
	reduced_diagonal(i)=rho_1p5(i,j)*mach
 enddo
enddo
...
do m=2,500
 do i=890,1112 !it is unecessary to calculate the change at the tails, suff small
   j=2002-i
        ...
        taud_inverse=(Pi/32.0d0)*taur_inverse*xsquared/(lambdadb)**2
        mach=dexp(-delt*taud_inverse)
	reduced_diagonal(i)=reduced_diagonal(i)*mach
 enddo
enddo
